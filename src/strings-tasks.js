...
function isString(value) {
  const verifString = typeof value === 'string' || value instanceof String;
  return verifString;
}
...
function repeatString(str, times) {
  let correctedTimes = times;
  if (times < 0) {
    correctedTimes = 0;
  }
  return str.repeat(correctedTimes);
}
...
function removeFirstOccurrences(str, value) {
  ...
  if (index >= 0) {
    const noSubstring = str.slice(0, index) + str.slice(index + value.length);
    return noSubstring;
  }
  return str;
}
...
function sumOfCodes(str) {
  const verifiedStr = str || '';
  const strLength = verifiedStr.length;
  let sum = 0;
...
function formatTime(minutes, seconds) {
  ...
  const formatted = `${formatMin}:${formatSec}`;
  return formatted;
}
...
function orderAlphabetically(str) {
  ...
  const orderedStr = strArray2.sort().join('');
  return orderedStr;
}
...
function countVowels(str) {
  ...
  const foundVowels = strFilter.filter((x) => vowelsArray.includes(x)).length;
  return foundVowels;
}
...
function isPalindrome(str) {
  ...
  const strArray3 = splitStr.filter((char) => /[a-zA-Z]/.test(char));
  const reversedStr2 = strArray3.slice().reverse().join();
  return strArray3.join() === reversedStr2;
}
...
function invertCase(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    if (char === char.toUpperCase()) {
      invertedCase += char.toLowerCase();
    } else {
      invertedCase += char.toUpperCase();
    }
  }
...
function extractNameFromTemplate(value) {
  ...
  const extractedName = [splitValue[1], splitValue[2].replace(/[^a-z]/i, '')];
  return extractedName.join(' ');
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, (char) => {
    const encodeChar = char <= 'Z' ? 'A'.charCodeAt(0) : 'a'.charCodeAt(0);
    return String.fromCharCode(
      encodeChar + ((char.charCodeAt(0) - encodeChar + 13) % 26)
    );
  });
}
